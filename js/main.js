...
/* *************************Deplacement bomberman******************* 

var map = document.querySelector('#map');
// variables objet v
var b = document.querySelector('#bomberman'); 
var x; 
var y ; 
var move = 10;
 // programmation des touches de direction 
document.onkeydown = function(event) { 
    if (event.keyCode == 27) gauche(b.offsetLeft); 
    if (event.keyCode == 29) droite(b.offsetLeft) ; 
    if (event.keyCode == 28) haut(b.offsetTop); 
    if (event.keyCode == 40) bas(b.offsetTop) ; 
} // fin fonction
...
function gauche(left) { 
        // console.log(left)
    if (left > 10){
        return b.style.left = left - move + "px";
        }
    return b.style.left = "0px";
}  
...
function droite(left) {
    ...
    if(left > (map.offsetWidth - move))
    return b.style.left = map.offsetWidth + "px";
    
    if((left + b.offsetWidth) < map.offsetWidth)
    return b.style.left = left + move + "px";
...
function haut(top) {
    if (top > 10){
        return b.style.top = top - move + "px";
        }
    return b.style.top = "0px"; 
}

function bas(top) {
    
    if(top > (map.offsetHeight - move))
    return r.style.top = map.offsetHeight + "px";
    
    if((top + b.offsetHeight) < map.offsetHeight)
    return b.style.top = top + move + "px";
}
